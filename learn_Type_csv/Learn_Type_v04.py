...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    def __init__(self, root: tk.Tk):
        ...
        self._mask_style: str = "block"   # "block" | "first" | "full"
    ...
    def _build_practice_panel(self, parent: tk.Frame):
        ...
        # ── 가리기 모드 전용 마스크 선택 바 (기본 숨김) ─────────────────────
        self._mask_bar = tk.Frame(parent, bg="#f0f2f5", pady=3, padx=12)
        ...
        self._mask_btn_first = tk.Button(
            ...
            command=self._toggle_mask_first, bg="#dfe6e9", fg="#2c3e50", **_mb
        )
        self._mask_btn_first.pack(side=tk.LEFT, padx=(0, 6))
        self._mask_btn_full = tk.Button(
            self._mask_bar, text="전체 가리기",
            command=self._toggle_mask_full, bg="#dfe6e9", fg="#2c3e50", **_mb
        )
    ...
    def _create_sentence_window(self):
        ...
        # 우측 버튼 컬럼 (RIGHT 먼저 pack 해야 paned 보다 오른쪽에 위치)
        ...
        tk.Button(btn_col, text="✏ 팝업 편집", command=self.edit_sentence,
                  bg="#8e44ad", fg="white", **_cb).pack(fill=tk.X, pady=2)
        ...
        # ── 인라인 수정 패널 (기본 숨김, PanedWindow 오른쪽 패널로 add/forget) ──
        self._inline_frame = tk.LabelFrame(
            self._paned, text=" 선택 문장 수정 ", font=self.fn_bold,
            bg="#fffde7", padx=4, pady=6, fg="#7d6608"
        )
        # (self._paned.add 는 _show_inline_edit 에서 호출)

        # 스크롤 가능한 내용 영역
        _inline_canvas = tk.Canvas(
            self._inline_frame, bg="#fffde7", bd=0, highlightthickness=0, width=300
        )
        _inline_vsb = ttk.Scrollbar(
            self._inline_frame, orient="vertical", command=_inline_canvas.yview
        )
        ...
        _inner.bind("<Configure>", lambda e: _inline_canvas.configure(
            scrollregion=_inline_canvas.bbox("all")))
        _inline_canvas.bind("<Configure>", lambda e: _inline_canvas.itemconfig(
            _inline_cwin, width=e.width))

        tk.Label(_inner, text="문장:", bg="#fffde7",
                 font=self.fn_sm).pack(anchor="w")
        self._inline_txt = tk.Text(
            _inner, height=2, font=self.fn_mono_sm, wrap=tk.WORD,
        ...
        tk.Label(_inner, text="해석:", bg="#fffde7",
                 font=self.fn_sm).pack(anchor="w")
        self._inline_desc = tk.Text(
            _inner, height=2, font=self.fn_sm, wrap=tk.WORD,
        ...
        tk.Label(_inner, text="설명:", bg="#fffde7",
                 font=self.fn_sm).pack(anchor="w")
    ...
    def _on_sent_double_click(self, event):
        ...
        if item and item.startswith("r_"):
            self.edit_sentence(item)
    ...
    def add_sentence(self):
        if not self.current_csv:
            ...
            return
        dlg = SentenceDialog(self.root, "문장 / 명령어 추가", font_size=self._font_size)
        if dlg.result:
    ...
    def _update_mask_buttons(self):
        """마스크 버튼 활성/비활성 색상을 현재 _mask_style 에 맞게 갱신."""
    ...
    def _toggle_mask_first(self):
        """첫글자만 보이기 / 기본(block) 토글."""
        self._mask_style = "block" if self._mask_style == "first" else "first"
        self._update_mask_buttons()
        typed = self.input_text.get("1.0", tk.END).rstrip("\n")
        self._refresh_target(typed)

    def _toggle_mask_full(self):
        """전체 가리기 / 기본(block) 토글."""
        self._mask_style = "block" if self._mask_style == "full" else "full"
        self._update_mask_buttons()
        typed = self.input_text.get("1.0", tk.END).rstrip("\n")
        self._refresh_target(typed)
    ...
    def _edit_current_sentence(self):
        """현재 연습 중인 문장을 팝업 창에서 편집하고 저장한다."""
        if not self.current_csv:
            messagebox.showinfo("안내", "먼저 파일 관리에서 CSV 파일을 선택하세요.")
            return
        if not self.practice_indices:
            messagebox.showinfo("안내", "연습할 문장이 없습니다.")
            return
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    def __init__(self, parent: tk.Misc,
        ...
        fn_sm = Font(family="Malgun Gothic", size=font_size)
        ...
        dlg_canvas = tk.Canvas(
            scroll_outer, bg="#f0f2f5", bd=0, highlightthickness=0,
        ...
        def _on_content_configure(e):
            dlg_canvas.configure(scrollregion=dlg_canvas.bbox("all"))
        content.bind("<Configure>", _on_content_configure)

        def _on_canvas_configure(e):
            dlg_canvas.itemconfig(_cwin, width=e.width)
        dlg_canvas.bind("<Configure>", _on_canvas_configure)

        # 마우스 휠 스크롤
        def _on_mousewheel(e):
            dlg_canvas.yview_scroll(int(-1 * (e.delta / 120)), "units")
        dlg_canvas.bind("<MouseWheel>", _on_mousewheel)
        content.bind("<MouseWheel>", _on_mousewheel)

        # ── 글자 수에 따라 입력란 높이 자동 조절 (제한 없음 — 외부 캔버스가 스크롤) ──
        def auto_h(widget: tk.Text, min_h: int = 2):
            def _do():
                try:
                    result = widget.count("1.0", "end", "displaylines")
                    lines  = result[0] if result else min_h
                    widget.configure(height=max(min_h, lines))
                    dlg.update_idletasks()
                except Exception:
                    pass
            widget.after_idle(_do)
        ...
        # 명령어/문장 입력
        tk.Label(content, text="명령어 / 문장:", bg="#f0f2f5", font=fn).pack(
        ...
        # 해석 입력
        tk.Label(content, text="해석 (선택 사항):", bg="#f0f2f5", font=fn).pack(
        ...
        # 설명 입력
        tk.Label(content, text="설명 (선택 사항):", bg="#f0f2f5", font=fn).pack(
            anchor="w", padx=14, pady=(10, 2)
        )
        ...
        tk.Label(content, text="Ctrl+Enter: 확인  /  Esc: 취소",
                 bg="#f0f2f5", fg="#888", font=fn_sm).pack(anchor="e", padx=14, pady=(4, 8))
        ...
        # 초기 내용에 맞게 높이 조절
        # dlg.update() 로 완전 렌더링 후 displaylines 정확히 계산
        dlg.update()
