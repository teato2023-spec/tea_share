"""
Memorization-Optimized Typing Practice v7
- Default mode: Mask (full block) — optimized for memorization
- Auto-switches to Review mode when SRS cards are due
- Translation/notes always visible to aid memorization
- SRS rating shortcuts: 1=Again  2=Hard  3=Good  4=Easy
- Top stats bar: today's due / memorized / total
- Auto-reveals answer text on completion
- CSV format: text, description, explanation
"""
...
_DEFAULT_BASE_DIR = os.path.join(os.path.expanduser("~"), "Documents", "Learn Type")
_SETTINGS_PATH   = os.path.join(os.path.expanduser("~"), ".config", "learn_type", "settings_v7.json")
...
_SHORTCUT_ACTIONS = [
    ...
    ("rate_2",  "Rate: Good"),
    ("rate_3",  "Rate: Easy"),
    ("memorize","Mark Memorized"),
    ("mask_first", "First-letter hint"),
    ("mask_full",  "Full mask"),
]
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    def __init__(self, root: tk.Tk):
        ...
        # memorization default mode = Mask
        self.typing_mode = tk.StringVar(value="Mask")
        self._mask_style: str = "full"   # default full mask
    ...
    def _open_shortcuts_window(self):
        ...
        def capture(action_id: str, lbl: tk.Label):
            dlg = tk.Toplevel(win)
            dlg.title("Press Key"); dlg.configure(bg="#f0f2f5")
            dlg.transient(win); dlg.grab_set(); dlg.resizable(False, False)
            tk.Label(dlg, text="Press the desired key combination", bg="#f0f2f5", font=self.fn_bold, pady=12, padx=24).pack()
            cur = self.shortcuts.get(action_id, "")
            disp_var = tk.StringVar(value=_bind_to_display(cur) or "(none)")
            captured = [cur]
            ...
            def confirm():
                ...
                lbl.config(text=_bind_to_display(captured[0]) or "(none)")
                dlg.destroy()
            def clear(): captured[0] = ""; disp_var.set("(none)")
            bf = tk.Frame(dlg, bg="#f0f2f5"); bf.pack(pady=(0,14))
            _b = dict(font=self.fn_sm, relief=tk.FLAT, padx=10, pady=3, cursor="hand2")
            tk.Button(bf, text="OK",    command=confirm,      bg="#27ae60", fg="white", **_b).pack(side=tk.LEFT, padx=3)
            tk.Button(bf, text="Clear", command=clear,        bg="#e67e22", fg="white", **_b).pack(side=tk.LEFT, padx=3)
            tk.Button(bf, text="Cancel", command=dlg.destroy, bg="#bdc3c7",             **_b).pack(side=tk.LEFT, padx=3)
        ...
        for row, (action_id, label) in enumerate(_SHORTCUT_ACTIONS, start=1):
            ...
            cur = self.shortcuts.get(action_id, "")
            key_lbl = tk.Label(body, text=_bind_to_display(cur) or "(none)",
        ...
        def reset_all():
            for aid in self.shortcuts:
                self.shortcuts[aid] = ""
                if aid in key_labels: key_labels[aid].config(text="(none)")

        bf = tk.Frame(win, bg="#f0f2f5", pady=10); bf.pack()
        _b = dict(font=self.fn_sm, relief=tk.FLAT, padx=12, pady=3, cursor="hand2")
        tk.Button(bf, text="Save", command=lambda: (self._apply_shortcuts(), self._save_settings(), win.destroy()),
                  bg="#27ae60", fg="white", **_b).pack(side=tk.LEFT, padx=4)
        tk.Button(bf, text="Reset All", command=reset_all, bg="#e67e22", fg="white", **_b).pack(side=tk.LEFT, padx=4)
        tk.Button(bf, text="Cancel", command=win.destroy, bg="#bdc3c7", **_b).pack(side=tk.LEFT, padx=4)

    def _open_options_window(self):
        win = tk.Toplevel(self.root)
        win.title("Options"); win.resizable(False, False)
        win.configure(bg="#f0f2f5"); win.transient(self.root); win.grab_set()

        body = tk.Frame(win, bg="#f0f2f5", padx=20, pady=16)
        body.pack(fill=tk.BOTH)

        tk.Label(body, text="Font Size", bg="#f0f2f5", font=self.fn_bold).grid(row=0, column=0, sticky="w", pady=(0,6))
        ...
        tk.Label(body, text="Base Folder", bg="#f0f2f5", font=self.fn_bold).grid(row=1, column=0, sticky="w", pady=(10,4))
        ...
        def change_dir():
            new_dir = filedialog.askdirectory(title="Select Base Folder", initialdir=self.base_dir, parent=win)
            if not new_dir: return
            self.base_dir = new_dir
            os.makedirs(self.base_dir, exist_ok=True)
            self._save_settings(); dir_lbl.config(text=self.base_dir); self.load_tree()

        tk.Button(body, text="Change Folder", command=change_dir,
        ...
        tk.Button(body, text="Shortcut Settings...", command=self._open_shortcuts_window,
                  font=self.fn_sm, relief=tk.FLAT, padx=10, pady=3,
    ...
    def _setup_ui(self):
        ...
        tk.Label(stat_bar, text="Memorize", bg="#16213e", fg="#e94560",
                 font=self.fn_bold).pack(side=tk.LEFT, padx=(0, 20))

        self._stat_review_lbl = tk.Label(
            stat_bar, text="Due 0", bg="#16213e", fg="#f5a623",
        ...
        self._stat_mem_lbl = tk.Label(
            stat_bar, text="Memorized 0 / 0", bg="#16213e", fg="#2ecc71",
            font=self.fn_stat
        )
        self._stat_mem_lbl.pack(side=tk.LEFT, padx=12)

        self._stat_file_lbl = tk.Label(
            stat_bar, text="No file", bg="#16213e", fg="#7f8c8d",
            font=self.fn_stat
        )
        ...
        tk.Button(stat_bar, text="Options", command=self._open_options_window,
                  bg="#2c3e50", fg="#bdc3c7", **_rb).pack(side=tk.RIGHT, padx=2)
        tk.Button(stat_bar, text="Files", command=self._open_tree_window,
                  bg="#2c3e50", fg="#bdc3c7", **_rb).pack(side=tk.RIGHT, padx=2)
        tk.Button(stat_bar, text="Sentences", command=self._open_sentence_window,
                  bg="#2c3e50", fg="#bdc3c7", **_rb).pack(side=tk.RIGHT, padx=2)
        ...
        tk.Button(nav, text="◀ Prev", command=self.prev_sentence, **_nb).pack(side=tk.LEFT)
        self._counter_lbl = tk.Label(nav, text="0 / 0", bg="#0f3460", fg="#f5a623", font=self.fn_bold)
        self._counter_lbl.pack(side=tk.LEFT, padx=10)
        tk.Button(nav, text="Next ▶", command=self.next_sentence, **_nb).pack(side=tk.LEFT)
        tk.Button(nav, text="Restart", command=self.restart_all, **_nb).pack(side=tk.LEFT, padx=(8,0))
        ...
        # mode selection
        _MODE_COLORS = {"Basic": "#3498db", "Mask": "#8e44ad", "Review": "#e67e22"}
        for mode in ["Basic", "Mask", "Review"]:
            tk.Radiobutton(
    ...
    def _build_practice_panel(self, parent: tk.Frame):
        ...
        self._mask_btn_first = tk.Button(
            self._mask_bar, text="First-letter",
        ...
        target_lf = tk.LabelFrame(
            parent, text=" Sentence ", font=self.fn_bold,
        ...
        # ── translation frame (memorization aid — always visible) ─────────────
        self._middle = tk.Frame(parent, bg="#1a1a2e")
        self._middle.pack(fill=tk.X)

        self._desc_frame = tk.LabelFrame(
            self._middle, text=" Translation ", font=self.fn_bold,
        ...
        self._desc_copy_btn = tk.Button(
            _desc_row, text="Copy", font=self.fn_stat,
        ...
        self._explanation_frame = tk.LabelFrame(
            self._middle, text=" Notes ", font=self.fn_bold,
        ...
        self._expl_copy_btn = tk.Button(
            _expl_row, text="Copy", font=self.fn_stat,
        ...
        input_lf = tk.LabelFrame(
            parent, text=" Input ", font=self.fn_bold,
            bg="#1a1a2e", fg="#e94560", padx=10, pady=6
        )
        ...
        tk.Label(self._banner_frame, text="Done!",
                 bg="#1e4d2b", fg="#2ecc71",
        ...
        bot.pack(fill=tk.X)
        tk.Button(bot, text="Memorized", command=self.memorize_current,
                  font=self.fn_sm, relief=tk.FLAT, padx=8, pady=3,
                  cursor="hand2", bg="#1e8449", fg="white").pack(side=tk.LEFT, padx=4)
        tk.Button(bot, text="Edit", command=self._edit_current_sentence,
                  font=self.fn_sm, relief=tk.FLAT, padx=8, pady=3,
    ...
    def _create_sentence_window(self):
        ...
        hdr.pack(fill=tk.X)
        tk.Label(hdr, text="Sentences", bg="#bdc3c7", font=self.fn_bold).pack(side=tk.LEFT)
        self._csv_lbl = tk.Label(hdr, text="Select a CSV from Files",
        ...
        tk.Label(search_frame, text="Search:", bg="#f0f2f5", font=self.fn_sm).pack(side=tk.LEFT)
        ...
        tk.Button(btn_col, text="+ Add",    command=self.add_sentence,   bg="#27ae60", fg="white", **_cb).pack(fill=tk.X, pady=2)
        tk.Button(btn_col, text="✏ Edit",   command=self.edit_sentence,  bg="#8e44ad", fg="white", **_cb).pack(fill=tk.X, pady=2)
        tk.Button(btn_col, text="Delete",   command=self.delete_sentence, bg="#e74c3c", fg="white", **_cb).pack(fill=tk.X, pady=2)
        tk.Label(btn_col, bg="#f8f9fa").pack(fill=tk.Y, expand=True)
        tk.Button(btn_col, text="Close",    command=win.withdraw,         bg="#7f8c8d", fg="white", **_cb).pack(fill=tk.X, pady=2)
        ...
        self.sent_tree.heading("check",   text="Done")
        self.sent_tree.heading("content", text="Text")
        self.sent_tree.heading("srs",     text="Level")
        ...
        self._inline_frame = tk.LabelFrame(
            self._paned, text=" Edit Selected ", font=self.fn_bold,
        ...
        tk.Label(_inner, text="Text:", bg="#fffde7", font=self.fn_sm).pack(anchor="w")
        ...
        tk.Label(_inner, text="Translation:", bg="#fffde7", font=self.fn_sm).pack(anchor="w")
        ...
        tk.Label(_inner, text="Notes:", bg="#fffde7", font=self.fn_sm).pack(anchor="w")
        ...
        tk.Button(ibf, text="Save",   command=self._save_inline_edit, bg="#27ae60", fg="white", **_ib).pack(side=tk.LEFT, padx=3)
        tk.Button(ibf, text="Cancel", command=self._hide_inline_edit, bg="#95a5a6", fg="white", **_ib).pack(side=tk.LEFT)

    def _open_sentence_window(self):
        if not self.current_csv:
            messagebox.showinfo("Info", "Please select a CSV from Files first.")
            return
    ...
    def _create_tree_window(self):
        ...
        tk.Label(win, text="  Folders / Files", bg="#bdc3c7", font=self.fn_bold, anchor="w", pady=5).pack(fill=tk.X)
        ...
        tk.Button(btn_f, text="New Folder",  command=self.create_folder,    bg="#e67e22", fg="white", **_bb).pack(side=tk.LEFT, padx=3)
        tk.Button(btn_f, text="New CSV",     command=self.create_csv_file,  bg="#27ae60", fg="white", **_bb).pack(side=tk.LEFT, padx=3)
        tk.Button(btn_f, text="Import",      command=self.import_csv,       bg="#3498db", fg="white", **_bb).pack(side=tk.LEFT, padx=3)
        tk.Button(win,   text="Close",       command=win.withdraw,          bg="#bdc3c7",             **_bb).pack(pady=(0,6))
    ...
    def _on_tree_drop(self, event):
        ...
        for f in files:
            ...
            if os.path.abspath(f) == os.path.abspath(dest):
                continue
            if os.path.exists(dest) and not messagebox.askyesno("Overwrite", f"'{os.path.basename(dest)}' already exists. Overwrite?", parent=self._tree_win):
                continue
            try:
                shutil.copy2(f, dest); imported += 1
            except Exception as e:
                messagebox.showerror("Error", str(e), parent=self._tree_win)
        if imported:
            self.load_tree()
            messagebox.showinfo("Done", f"{imported} CSV file(s) imported.", parent=self._tree_win)
    ...
    def _insert_dir(self, path: str, parent: str):
        ...
        for name in entries:
            ...
            if os.path.isdir(full):
                ...
                node = self.tree.insert(parent, tk.END, text=f"[Dir] {name}", values=[full], tags=("folder",), open=False)
    ...
    def _show_tree_ctx_menu(self, event):
        ...
        menu.add_command(label="New CSV",      command=self.create_csv_file)
        ...
        menu.add_command(label="Save",         command=self._save_csv,    state=s)
        menu.add_command(label="Save As",      command=self.save_csv_as,  state=s)
        menu.add_separator()
        menu.add_command(label="Rename",       command=self.rename_tree_item)
        menu.add_command(label="Delete",       command=self.delete_tree_item)
        menu.tk_popup(event.x_root, event.y_root)

    # ══════════════════════════════════════════════════════════════════════════
    # folder / CSV file management
    # ══════════════════════════════════════════════════════════════════════════

    def create_folder(self):
        name = self._ask_string("New Folder", "Folder name:")
        if not name: return
        try:
    ...
    def create_csv_file(self):
        name = self._ask_string("New CSV", "File name (auto .csv):")
        if not name: return
    ...
    def import_csv(self):
        src = filedialog.askopenfilename(
            title="Select CSV to Import", initialdir=self.base_dir,
            filetypes=[("CSV files", "*.csv"), ("All files", "*.*")]
        )
        if not src: return
        dest = os.path.join(self._get_target_dir(), os.path.basename(src))
        if os.path.exists(dest) and not messagebox.askyesno("Overwrite", f"'{os.path.basename(dest)}' already exists. Overwrite?"):
            return
        try:
    ...
    def rename_tree_item(self):
        ...
        new = self._ask_string("Rename", "New name:", initialvalue=old)
    ...
    def delete_tree_item(self):
        ...
        if not path:
            ...
            return
        kind = "folder (with contents)" if os.path.isdir(path) else "file"
        if not messagebox.askyesno("Confirm Delete", f"Delete {kind} '{os.path.basename(path)}'?"):
            return
        try:
        ...
        try:
            ...
            with open(path, newline="", encoding="utf-8-sig") as f:
                ...
                if header:
                    ...
                    for alias in ("description", "description", "desc"):
                        if alias in low:
                            desc_col = low.index(alias); has_desc = True; break
                    for alias in ("explanation", "explanation", "expl"):
                        if alias in low:
            ...
            self._sent_win.title(f"Sentences — {fname}")
            self._stat_file_lbl.config(text=fname)
            self._refresh_sent_tree()
            # auto-switch to review mode if review cards exist
    ...
    def _auto_select_mode(self):
        """Switch to review mode if review cards are due, otherwise mask mode."""
        ...
        if review_count > 0:
            self.typing_mode.set("Review")
        else:
            self.typing_mode.set("Mask")
    ...
    def save_csv_as(self):
        ...
        try:
            ...
            self._csv_lbl.config(text=fname, fg="#2c3e50")
            self._sent_win.title(f"Sentences — {fname}")
            messagebox.showinfo("Saved", f"'{fname}' saved.", parent=self._tree_win)
        except Exception as e:
            messagebox.showerror("Save Error", str(e), parent=self._tree_win)
    ...
    def _refresh_sent_tree(self):
        ...
        if not active:
            self.sent_tree.insert("", tk.END, iid="empty_active",
                                  values=("","    (empty)",""), tags=("empty",))
        self.sent_tree.insert("", tk.END, iid="hdr_done",
                              values=("","  Memorized",""), tags=("hdr",))
    ...
    def _save_inline_edit(self):
        ...
        if not text:
            messagebox.showwarning("Warning", "Please enter text.", parent=self._sent_win); return
    ...
    def add_sentence(self):
        if not self.current_csv:
            messagebox.showinfo("Info", "Please select or create a CSV from Files first."); return
        dlg = SentenceDialog(self.root, "Add Sentence", font_size=self._font_size)
        if dlg.result:
    ...
    def edit_sentence(self, iid: str | None = None):
        ...
        if not iid or not iid.startswith("r_"):
            messagebox.showinfo("Info", "Select a sentence to edit."); return
        idx = int(iid[2:])
        row = self.sentence_data[idx]
        text = row[0]; desc = row[1] if len(row) > 1 else ""; expl = row[2] if len(row) > 2 else ""
        dlg = SentenceDialog(self.root, "Edit Sentence", text, desc, expl, font_size=self._font_size)
        if dlg.result:
            old_text = self.sentence_data[idx][0]
    ...
    def delete_sentence(self):
        sel = self.sent_tree.selection()
        if not sel: messagebox.showinfo("Info", "Select an item to delete."); return
        iid = sel[0]
        if not iid.startswith("r_"): messagebox.showinfo("Info", "Select a sentence to delete."); return
        idx = int(iid[2:])
        preview = self.sentence_data[idx][0]
        preview = preview if len(preview) <= 40 else preview[:37] + "…"
        if not messagebox.askyesno("Confirm Delete", f"Delete this sentence?\n\"{preview}\""): return
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # practice order / random
    # ══════════════════════════════════════════════════════════════════════════

    def _build_practice_indices(self):
        if self.typing_mode.get() == "Review":
            today = date.today().isoformat()
            indices = [
    ...
    def _switch_to_review(self):
        """Click on stats bar review counter → switch to review mode."""
        self.typing_mode.set("Review")
    ...
    def _update_stat_bar(self):
        ...
        self._stat_review_lbl.config(
            text=f"📅 Due {review_due}",
            fg="#f5a623" if review_due > 0 else "#5d6d7e"
        )
        self._stat_mem_lbl.config(text=f"Memorized {mem} / {total}")
    ...
    def reset_current(self):
        ...
        # mask/review mode: always show translation
        mode = self.typing_mode.get()
        if mode in ("Mask", "Review"):
    ...
    def _clear_all(self):
        ...
        self.srs_data         = {}
        self._csv_lbl.config(text="Select a CSV from Files", fg="#555")
        self._sent_win.title("Sentences")
        self._stat_file_lbl.config(text="No file")
        self._refresh_sent_tree()
    ...
    def _update_counter(self):
        ...
        prefix = "📅 " if self.typing_mode.get() == "Review" else ""
        self._counter_lbl.config(text=f"{prefix}{cur} / {total}")

    # ── banner / description show·hide ───────────────────────────────────────
    ...
    def _copy_desc(self):
        ...
        if text:
            ...
            self._desc_copy_btn.config(text="✓ Copied")
            self.root.after(1500, lambda: self._desc_copy_btn.config(text="Copy"))

    def _copy_explanation(self):
        text = self._explanation_lbl.cget("text")
        if text:
            self.root.clipboard_clear(); self.root.clipboard_append(text)
            self._expl_copy_btn.config(text="✓ Copied")
            self.root.after(1500, lambda: self._expl_copy_btn.config(text="Copy"))
    ...
    # ── practice sentence rendering ───────────────────────────────────────────

    def _on_mode_change(self):
        self._mask_style = "full"   # reset to full mask on mode change
        self._update_mask_buttons()
        mode = self.typing_mode.get()
        self._build_practice_indices()
        if not self.practice_indices:
            self._clear_practice_area()
            if mode == "Review" and self.sentence_data:
                messagebox.showinfo("Review Complete", "No sentences due for review today.\nCheck back tomorrow!")
            return
        self.reset_current()
        self._update_stat_bar()
    ...
    def _handle_enter(self, _e):
        mode = self.typing_mode.get()
        if mode in ("Mask", "Review") and self.practice_indices:
    ...
    def _handle_up(self, _e):
        if self.typing_mode.get() not in ("Mask", "Review") or not self.practice_indices:
            return "break"
    ...
    def _toggle_reveal(self):
        """Toggle reveal/hide of answer in mask mode (for shortcut binding)."""
        if not self.practice_indices: return
        if self.typing_mode.get() not in ("Mask", "Review"): return
    ...
    def _check_completion(self, typed: str):
        if typed == self._cur_text() and not self.completed:
            ...
            if mode in ("Mask", "Review"):
                self._show_banner()
            # reveal answer text on completion
            self.target_display.config(state=tk.NORMAL)
            self.target_display.delete("1.0", tk.END)
    ...
    def memorize_current(self):
        ...
        if not self.practice_indices:
            self._clear_practice_area()
            messagebox.showinfo("Complete", "All sentences memorized!")
            return
    ...
    def next_sentence(self, _e=None):
        ...
        if self.current_index < len(self.practice_indices) - 1:
            ...
            if self.typing_mode.get() == "Review":
                messagebox.showinfo("Review Complete", "Today's review is complete!")
            else:
                messagebox.showinfo("Complete", "All sentences complete!")
    ...
    def _edit_current_sentence(self):
        if not self.current_csv:
            messagebox.showinfo("Info", "Please select a CSV from Files first."); return
        if not self.practice_indices:
            messagebox.showinfo("Info", "No sentences to practice."); return
        data_idx = self.practice_indices[self.current_index]
        row  = self.sentence_data[data_idx]
        text = row[0]; desc = row[1] if len(row)>1 else ""; expl = row[2] if len(row)>2 else ""
        dlg = SentenceDialog(self.root, "Edit Current Sentence", text, desc, expl, font_size=self._font_size)
        if dlg.result:
            self._transfer_srs(text, dlg.result[0])
            self.sentence_data[data_idx] = dlg.result
            self._save_csv(); self._refresh_sent_tree(); self.reset_current()
            if self.typing_mode.get() in ("Mask", "Review"):
                self._update_desc_display()
    ...
    def _ask_string(self, title: str, prompt: str, initialvalue: str = "") -> str | None:
        ...
        tk.Button(bf, text="OK",     command=ok,     bg="#3498db", fg="white", **_b).pack(side=tk.LEFT, padx=6)
        tk.Button(bf, text="Cancel", command=cancel, bg="#95a5a6", fg="white", **_b).pack(side=tk.LEFT)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    def __init__(self, parent: tk.Misc,
        ...
        tk.Label(content, text="Text:", bg="#f0f2f5", font=fn).pack(anchor="w", padx=14, pady=(14,2))
        ...
        tk.Label(content, text="Translation (optional):", bg="#f0f2f5", font=fn).pack(anchor="w", padx=14, pady=(10,2))
        ...
        tk.Label(content, text="Notes (optional):", bg="#f0f2f5", font=fn).pack(anchor="w", padx=14, pady=(10,2))
        ...
        tk.Label(content, text="Ctrl+Enter: Confirm  /  Esc: Cancel",
        ...
        def confirm(_e=None):
            ...
            else: messagebox.showwarning("Warning", "Please enter text.", parent=dlg)

        def cancel(_e=None): dlg.destroy()

        bf = tk.Frame(dlg, bg="#f0f2f5"); bf.pack(pady=10)
        _b = dict(font=fn, relief=tk.FLAT, padx=14, pady=4, cursor="hand2")
        tk.Button(bf, text="OK",     command=confirm, bg="#3498db", fg="white", **_b).pack(side=tk.LEFT, padx=6)
        tk.Button(bf, text="Cancel", command=cancel,  bg="#95a5a6", fg="white", **_b).pack(side=tk.LEFT)
