...
class SentenceEditDialog:
    def __init__(self, parent, initial: str = ""):
        ...
        tk.Label(dlg, text="문장을 입력하세요:",
                 bg=C["panel"], fg=C["fg2"],
                 font=("맑은 고딕", 9)).pack(anchor="w", padx=10, pady=(10, 2))

        txt_outer = tk.Frame(dlg, bg=C["border"], padx=1, pady=1)
        txt_outer.pack(fill="both", expand=True, padx=10, pady=(0, 6))
        ...
        self._txt = tk.Text(
            ...
            relief="flat", width=42, height=10,
            font=("맑은 고딕", 10), wrap="word",
            padx=6, pady=4,
        )
        sb = ttk.Scrollbar(txt_outer, orient="vertical", command=self._txt.yview)
        ...
        ttk.Button(btn_row, text="확인",
                   command=lambda: self._ok(dlg)).pack(side="left", padx=6)
    ...
    def _ok(self, dlg):
        ...
        if not text:
            messagebox.showwarning("알림", "문장을 입력하세요.", parent=dlg)
            return
        self.result = text
        dlg.destroy()
...
class AreaLayoutApp:
    ...
    def _build_ui(self):
        ...
        # ── 세 번째 칸: 문장 목록
        sentence_pane = ttk.LabelFrame(self._paned_left, text="문장 목록", padding=4)
        self._paned_left.add(sentence_pane, stretch="always", minsize=80)

        sent_frame = tk.Frame(sentence_pane, bg=C["panel"])
        ...
        self._sent_lb = tk.Listbox(
            ...
            font=("맑은 고딕", 9), height=4,
        )
        self._sent_lb.pack(fill="both", expand=True, side="left")
        sent_sb = ttk.Scrollbar(sent_frame, orient="vertical",
                                 command=self._sent_lb.yview)
        sent_sb.pack(side="left", fill="y")
        self._sent_lb.config(yscrollcommand=sent_sb.set)
        self._sent_lb.bind("<Double-Button-1>", lambda e: self._edit_sentence())

        sent_btn_row = tk.Frame(sentence_pane, bg=C["panel"])
        sent_btn_row.pack(fill="x", pady=(4, 0))
        ttk.Button(sent_btn_row, text="추가",
                   command=self._add_sentence).pack(side="left", expand=True,
                                                    fill="x", padx=2)
        ttk.Button(sent_btn_row, text="편집",
                   command=self._edit_sentence).pack(side="left", expand=True,
                                                     fill="x", padx=2)
        ttk.Button(sent_btn_row, text="삭제",
                   command=self._del_sentence).pack(side="left", expand=True,
                                                    fill="x", padx=2)
    ...
    def _dict_to_canvas(self, data):
        ...
        for rd in data.get("rulers", []):
            self.rulers.append(Ruler(rd["x1"], rd["y1"], rd["x2"], rd["y2"],
                                     rd.get("color", "")))
        self.sentences = list(data.get("sentences", []))
        self._sel_idx = -1
    ...
    def _refresh_sentence_list(self):
        self._sent_lb.delete(0, "end")
        for sent in self.sentences:
            preview = sent.replace("\n", " ")
            if len(preview) > 50:
                preview = preview[:50] + "…"
            self._sent_lb.insert("end", preview)

    def _add_sentence(self):
        dlg = SentenceEditDialog(self.root)
        if dlg.result:
            self.sentences.append(dlg.result)
            self._refresh_sentence_list()
            self._sent_lb.selection_clear(0, "end")
            self._sent_lb.selection_set("end")
            self._mark_dirty()
    ...
    def _edit_sentence(self):
        ...
        if not sel:
            return
        idx = sel[0]
        dlg = SentenceEditDialog(self.root, initial=self.sentences[idx])
        if dlg.result:
            self.sentences[idx] = dlg.result
            self._refresh_sentence_list()
            self._sent_lb.selection_set(idx)
            self._mark_dirty()

    def _del_sentence(self):
        sel = self._sent_lb.curselection()
        if not sel:
            return
        idx = sel[0]
        if not messagebox.askyesno("삭제 확인", "선택한 문장을 삭제할까요?",
                                   parent=self.root):
            return
